...
\\
\\
Communication between peers and the media server is pure \gls{rtp} streams with all the information that is needed provided in the header of the packets being sent. The most important piece of information is the \gls{ssrc} with this, the media server can identify the incoming stream and mix it in with the other streams and forward it to all the other peers.

So basically all we need is to be able to listen for incoming \gls{rtp} streams on the receiving end. And the other way we have to deliver pure \gls{rtp} streams with the correct header information included, most importantly the \gls{ssrc}.
